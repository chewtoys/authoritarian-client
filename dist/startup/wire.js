...
export async function wire({ debug, profileMagistrate: profiler, tokenStorage, paywallGuardian, loginPopupRoutine, decodeAccessToken, privateVimeoGovernor, userPanels, profilePanels, paywallPanels, privateVimeos, avatarDisplays, }) {
    ...
    profile.reader.subscribe(state => {
        ...
        if (picture)
            avatar.wiring.setPictureUrl(picture);
    });
    ...
    for (const privateVimeo of privateVimeos) {
        ...
        if (!videoName)
            err(`<private-vimeo> is missing attribute [video-name]`);
